/****************************************************************************************************************************
   ...
   For AVR or Generic boards using WIFI_GENERIC modules/shields, using much less code to support boards with smaller memory
...
// Stating positon to store WIFI_GENERIC_config
#define CONFIG_EEPROM_START    (EEPROM_START + DRD_FLAG_DATA_SIZE)
    ...
    bool checkDynamicData()
    {
      ...
      {       
        ...
        WG_LOGERROR3(F("ChkCrR:pdata="), readBuffer, F(",len="), myMenuItems[i].maxlen);     
    ...
    {   
      ...
      String WIFI_GENERIC_FLDSET_START = "<fieldset>";
      String WIFI_GENERIC_FLDSET_END     = "</fieldset>";
      String WIFI_GENERIC_HTML_PARAM    = "<div><label>{b}</label><input value='[[{v}]]'id='{i}'><div></div></div>";
      String WIFI_GENERIC_HTML_BUTTON    = "<button onclick=\"sv()\">Save</button></div>";
      String WIFI_GENERIC_HTML_SCRIPT    = "<script id=\"jsbin-javascript\">function udVal(key,val){var request=new XMLHttpRequest();var url='/\
...
udVal('nm',document.getElementById('nm').value);";
      ...
      String WIFI_GENERIC_HTML_SCRIPT_ITEM   = "udVal('{d}',document.getElementById('{d}').value);";

      String WIFI_GENERIC_HTML_SCRIPT_END   = "alert('Updated');}</script>";

      String WIFI_GENERIC_HTML_END   = "</html>";
      ///
      ...
      root_html_template += WIFI_GENERIC_HTML_BUTTON;
      root_html_template += WIFI_GENERIC_HTML_SCRIPT;

#if USE_DYNAMIC_PARAMETERS      
      for (uint16_t i = 0; i < NUM_MENU_ITEMS; i++)
      {
        pitem = WIFI_GENERIC_HTML_SCRIPT_ITEM;
      ...
      root_html_template += WIFI_GENERIC_HTML_SCRIPT_END;
      root_html_template += WIFI_GENERIC_HTML_END;
      ...
      if (server)
      {
        ...
        if (key == "" && value == "")
        {
          ...
          else if ( WIFI_GENERIC_config.board_name[0] != 0 )
          {
            ...
            result.replace("AVR_WM_NINA_Lite", WIFI_GENERIC_config.board_name);
          }
